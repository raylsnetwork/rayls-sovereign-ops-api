...
- Integration tests: `ory/dockertest` with PostgreSQL, `//go:build ignore` tag

```go
func TestUserService_GetByID_ReturnsUser(t *testing.T) {
    // Querying a user by ID returns the matching user
    repo := testutil.NewFakeUserRepository()
    repo.Users = []domain.User{buildUser("abc", "Alice")}

    svc := NewUserService(repo, &testutil.StubLogger{})
