...
For the full local stack against a RayUp-provisioned chain (ops-api + worker + custody + playground), see [`docs/flows.md`](docs/flows.md) and run `./start_dev.sh` (needs a local RayUp control plane on k3d). Note that this dev stack also depends on components that are not open-sourced yet (RayUp, the playground, and the Blockscout fork), so it cannot be brought up from the public repositories alone.
